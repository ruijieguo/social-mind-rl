...
def _cell(all_results: dict, m: str, bench: str, proto: str) -> tuple[list[dict], str]:
    """Unwrap the (records, source-tag) tuple. Source can be 'v2', 'v1-fallback', or 'missing'."""
    val = all_results.get(m, {}).get(bench, {}).get(proto)
    if val is None:
        return [], "missing"
    if isinstance(val, tuple):
        return val
    return val, "v2"  # legacy raw-list


def build_main_table(all_results: dict, v1_results: dict | None = None) -> str:
    """all_results[model][benchmark][protocol] = (list[record], source-tag)
    Optionally diff against v1_results[model][benchmark][protocol] = list[record]."""
    lines = []
    ...
    for bench in BENCHMARKS:
        for proto in PROTOCOLS:
            ...
            for m in MODELS:
                recs, src = _cell(all_results, m, bench, proto)
                a, c, n = acc(recs)
                if n == 0:
                    row.append("—")
                else:
                    ...
                    if v1_results is not None:
                        v1_recs = v1_results.get(m, {}).get(bench, {}).get(proto, [])
                        if v1_recs:
                            v1a, _, _ = acc(v1_recs)
                            d = a - v1a
                            sign = "+" if d >= 0 else ""
                            cell += f" ({sign}{d*100:.2f}pp)"
                    if src == "v1-fallback":
                        cell += " *(v1)*"
                    row.append(cell)
...
def build_per_task_tombench(all_results: dict) -> str:
    ...
    for m in MODELS:
        for proto in PROTOCOLS:
            recs, _ = _cell(all_results, m, "tombench", proto)
            any_recs.extend(recs)
    ...
    for proto in PROTOCOLS:
        ...
        for t in tasks:
            row = [t]
            for m in MODELS:
                cell_recs, _ = _cell(all_results, m, "tombench", proto)
                recs = [r for r in cell_recs if r.get("task") == t]
                a, c, n = acc(recs)
...
def build_per_order_hitom(all_results: dict) -> str:
    """Per-order breakdown for Hi-ToM."""
    lines = []
    for proto in PROTOCOLS:
        any_recs = []
        for m in MODELS:
            recs, _ = _cell(all_results, m, "hitom", proto)
            any_recs.extend(recs)
        ...
        for o in orders:
            row = [o]
            for m in MODELS:
                cell_recs, _ = _cell(all_results, m, "hitom", proto)
                recs = [r for r in cell_recs if r.get("task") == o]
                a, c, n = acc(recs)
                row.append("—" if n == 0 else f"{a:.4f} ({c}/{n})")
            lines.append("| " + " | ".join(row) + " |")
    return "\n".join(lines)
...
def build_per_language(all_results: dict) -> str:
    ...
    for proto in PROTOCOLS:
        ...
        for lang in ["en", "zh"]:
            ...
            for m in MODELS:
                cell_recs, _ = _cell(all_results, m, "tombench", proto)
                recs = [r for r in cell_recs if r.get("language") == lang]
                a, c, n = acc(recs)
                row.append("—" if n == 0 else f"{a:.4f} ({c}/{n})")
            lines.append("| " + " | ".join(row) + " |")
    return "\n".join(lines)
...
def build_invalid_pred_table(all_results: dict) -> str:
    ...
    for m in MODELS:
        for bench in BENCHMARKS:
            for proto in PROTOCOLS:
                recs, _ = _cell(all_results, m, bench, proto)
                if not recs:
                    continue
...
def main():
    ...
    # ----- Load v2 results (preferred) and v1 results (for delta comparison) -----
    all_results: dict = {m: {b: {} for b in BENCHMARKS} for m in MODELS}
    v1_results: dict = {m: {b: {} for b in BENCHMARKS} for m in MODELS}
    file_map = {
        "qwen3-8b-base": "base",
        "qwen3-8b-v10":  "v10",
        "qwen3-8b-api":  "dashscope",
    }
    for m, fbase in file_map.items():
        for bench in BENCHMARKS:
            # v2 first, fallback to v1
            v2_path = results_dir / bench / f"{fbase}_v2.json"
            v1_path = results_dir / bench / f"{fbase}.json"

            v2_recs = load(v2_path)
            v1_recs = load(v1_path)

            v2_by_proto: dict = defaultdict(list)
            for r in v2_recs:
                v2_by_proto[r["protocol"]].append(r)
            v1_by_proto: dict = defaultdict(list)
            for r in v1_recs:
                v1_by_proto[r["protocol"]].append(r)

            # Main results: prefer v2 cell-by-cell; fallback to v1 (with note)
            merged: dict = {}
            for proto in PROTOCOLS:
                if v2_by_proto.get(proto):
                    merged[proto] = (v2_by_proto[proto], "v2")
                elif v1_by_proto.get(proto):
                    merged[proto] = (v1_by_proto[proto], "v1-fallback")
                else:
                    merged[proto] = ([], "missing")

            all_results[m][bench] = merged
            v1_results[m][bench] = dict(v1_by_proto)

    # ----- Build report -----
    md = []
    md.append(f"# Qwen3-8B 全量评测报告 v2 (修复版) — base × v1.0 × DashScope API\n")
    md.append(f"> **日期**: {today}（v2 重测）\n")
    md.append("> **修复内容（v1 → v2）**：")
    md.append(">")
    md.append("> 1. **max_tokens 不足**: cot 4096→8192, direct_think 2048→8192（修复 base 模型 4.4% 题目截断 bug，详见 v1 报告 §8.9）")
    md.append("> 2. **ToMBench ZH 选项前缀重复**: ZH 字段已含 `A./B./C./D.` 前缀，模板再加一遍变成 `A. A. ...`；v2 自动 strip 已有前缀")
    md.append("> 3. **DashScope reasoning_content 收集**: stream 中 `delta.reasoning_content` 现在拼成 `<think>...</think>` 注入 content，便于审计（不影响 extractor 输出）")
    md.append("> 4. **vLLM max_model_len** 从 8192 → 16384（容纳 8192 max_tokens + 8192 input）")
    md.append("> 5. **跳过 del_tom**（用户 2026-05-29 指定）")
    md.append(">")
    ...
    md.append("> - **direct_think** (default-think): T=0, top_p=1, **max_tokens=8192**, enable_thinking=true (与历史 production_frozen 0.7450 口径对齐)")
    md.append("> - **cot**: T=0.6, top_p=0.95, **max_tokens=8192**, enable_thinking=true, 1 sample, cot system prompt")
    md.append("")
    md.append("> **部署**: 服务器 `h800@172.16.120.191` (8×H800)，base 占 GPU 0-3、v1.0 占 GPU 4-7，各 4 个 vLLM 实例 (TP=1, **max_model_len=16384**, gpu_util=0.85)。DashScope 走公网 API。")
    md.append("")
    md.append("---\n")
