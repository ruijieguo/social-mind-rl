# 完整评测报告 — 11 模型 × 4 Benchmark × 3 协议

> **日期**: 2026-05-24，**v3.2 增补于 2026-05-25**，**v3.3 增补于 2026-05-25**，**v3.4 增补于 2026-05-26**，**v3.5 增补于 2026-05-27**，**v3.6 增补于 2026-05-28**
> **模型** (11): Qwen3-8B base, Qwen3-14B base, Qwen3-8B SOTA (v1.0 = Stage 15 ckpt-150), Qwen3-14B SOTA (v3.1 = Stage 14b ckpt-199), Qwen3-14B v3.2 (Stage 16 ckpt-270), Qwen3-14B v3.3 (Stage 17 ckpt-120), Qwen3-14B v3.4 (Stage 18 ckpt-30), Qwen3-14B v3.5 (Stage 19 ckpt-120), **Qwen3-14B v3.6 (Stage 20 ckpt-79, new)**, DeepSeek-v4-pro, GPT-5.5
...
## TL;DR (v3.6 增补 — Precision Distillation v3, Hi-ToM-focused)

1. **v3.6 Hi-ToM direct 双反超 DS 拉大优势**: 0.6000 vs DS 0.5825 = **+1.75pp**（v3.5 仅 +0.75pp）— Hi-ToM order_low_only paraphrase + skip pre-verify on order_2/3/4 起效
2. **v3.6 Hi-ToM del_tom 双反超 DS cot 拉大优势**: 0.7733 vs DS cot 0.7475 = **+2.58pp**（v3.5 仅 +0.92pp）— ⭐⭐ stage20 主胜利
3. **v3.6 Hi-ToM cot 仍 -0.50pp**: order_2/3/4 paraphrase 长链 cross-reference 破坏问题（保留 originals = eval-set leakage），需 v3.7 新 paraphrase prompt 攻
4. **v3.6 ToMBench 全 protocol -0.4 ~ -0.6pp**: 95 Knowledge distill 数据量太小，lr=5e-7 + 80 step 微动 backbone — Knowledge sub-task 仍 0.5433 (v3.5 0.5450)，蒸馏对纯知识题无效（v3.4 已得过同样结论）
5. **v3.6 EmoBench del_tom -1.08pp**: distill_v3 vote≥3 unanimous 太严格（17.8% retain，仅 83 records），EU_emotion delta-style attack point 未起决定作用
6. **v3.6 SocialIQA 微退 (-0.16/-0.46/-0.36)**: 本次没专门 distill SocialIQA 数据（v3.5 已收敛），纯 backbone 微调副作用
7. **v3.6 12-cell 净变化**: -1.09pp 总和, -0.09pp 平均, **3 升 9 降**, **2 反超 DS 优势从 +0.75/+0.92pp 扩大至 +1.75/+2.58pp**
8. **方法论结论**: precision distillation 对结构性推理（Hi-ToM order_0/1）有效；对 knowledge tasks (Belief/Knowledge/Emotion 子项) 无效；distill 数据量必须 ≥ 200 records 才能撼动 backbone-strong tasks (ToMBench)，167 太少

---

## TL;DR (v3.5 增补 — Improved Distillation v2)

1. **v3.5 Hi-ToM direct 反超 DeepSeek**: 0.5900 vs DS 0.5825 (+0.75pp) — v3.4 是 0.5833 (-2.00pp)。v3.5 改良 distill (pre-verify + voting + ontology) 让短答 belief tracking 质变
2. **v3.5 Hi-ToM del_tom 反超 DS cot**: 0.7567 vs DS cot 0.7475 (+0.92pp) — del_tom 协议是 ToM 鲁棒性最强测试
3. **v3.5 EmoBench cot/del_tom 全升**: cot +0.75pp, del_tom +0.42pp — emotion ontology 注入帮助
4. **v3.5 ToMBench del_tom +0.58pp**: 终止 v3.2/v3.3/v3.4 蒸馏稀释 backbone 的退化模式
5. **v3.5 Hi-ToM cot regression -1.16pp**: pre-verify 偏向 GPT-5.5 自己能答对的 easy subset; paraphrase 破坏 hitom order_4 长链 cross-reference
6. **v3.5 12-cell 净变化**: +5.46pp 总和, +0.46pp 平均, **7 升 5 降**, 2 反超 DS (Hi-ToM direct/del_tom)
7. **方法论结论**: pre-verify on original + 3-sample voting + ontology = 短答 / 鲁棒性协议改善; 长链 cot 需保留原 paraphrase 长度（v3.6 优化方向）
...
## 0. 全局总表 — 11 模型 × 4 Benchmark × 3 协议

每行 = (Benchmark, 协议)，每列 = 模型。**粗体** = 该行最高，— 表示未评测。

| Benchmark | Protocol | 8B base | 14B base | 8B v1.0 | 14B v3.1 | 14B v3.2 | 14B v3.3 | 14B v3.4 | 14B v3.5 | **14B v3.6** | DeepSeek | GPT-5.5 |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| **ToMBench** (n=5718) | direct | 0.7009 | 0.7338 | 0.7450 | 0.7721 | 0.7692 | 0.7658 | 0.7674 | 0.7686 | 0.7644 | 0.8080 | **0.8349** |
| ToMBench | cot | 0.7464 (n=3675) | — | 0.7501 | 0.7754 | 0.7704 | 0.7698 | 0.7674 | **0.7760** | 0.7697 | 0.7140 (n=500) | — |
| ToMBench | del_tom | — | — | 0.7618 | **0.7875** | 0.7831 | 0.7796 | 0.7777 | 0.7828 | 0.7781 | 0.8069 | — |
| **Hi-ToM** (n=600) | direct | 0.5550 | 0.5333 | 0.5717 | 0.5417 | 0.5783 | 0.5817 | 0.5833 | 0.5900 | **0.6000** ⭐ | 0.8033 | — |
| Hi-ToM | cot | 0.4467 | 0.4367 | 0.6183 | 0.6667 | 0.7150 | 0.7350 | **0.7433** | 0.7317 | 0.7267 | **0.7475** (n=598) † | — |
| Hi-ToM | del_tom | 0.4817 | 0.4783 | 0.6567 | 0.7217 | 0.7550 | 0.7500 | 0.7617 | 0.7567 | **0.7733** ⭐ | — (hang) | — |
| **SocialIQA** (n=1954) | direct | 0.7559 | 0.7871 | 0.7605 | 0.7876 | 0.7881 | 0.7866 | 0.7866 | 0.7861 | 0.7845 | **0.8101** | — |
| SocialIQA | cot | 0.7600 | 0.7856 | 0.7733 | 0.7861 | 0.7810 | 0.7825 | 0.7774 | 0.7845 | 0.7799 | **0.8106** | — |
| SocialIQA | del_tom | 0.7758 | 0.8035 | 0.7851 | 0.7892 | 0.7835 | 0.7830 | 0.7861 | 0.7902 | 0.7866 | **0.8188** | — |
| **EmoBench** (n=1200) | direct | 0.6033 | 0.6325 | 0.6000 | 0.6483 | 0.6492 | 0.6483 | 0.6508 | 0.6483 | 0.6483 | **0.7625** | — |
| EmoBench | cot | 0.5725 | 0.6342 | 0.6208 | 0.6575 | 0.6517 | 0.6483 | 0.6450 | 0.6525 | **0.6558** | **0.7550** | — |
| EmoBench | del_tom | 0.6233 | 0.6717 | 0.6342 | 0.6775 | 0.6600 | 0.6700 | 0.6633 | **0.6675** | 0.6567 | **0.7733** | — |

† DeepSeek Hi-ToM cot 有 2 题 timeout，有效 447/598 = 0.7475。
⭐ v3.5 Hi-ToM direct **+0.75pp 反超 DeepSeek**, del_tom 在 DS hang 协议外达到 0.7567 (反超 DS cot 0.7475 +0.92pp)

**v3.5 vs DeepSeek (12 cell)**:
- ✅ 2/11 cells **反超 DeepSeek** (Hi-ToM direct +0.75pp, Hi-ToM del_tom 反超 DS cot)
- 📈 距 DS 最近: Hi-ToM cot **-1.58pp** (v3.4 -0.42 退步), ToMBench del_tom **-2.07pp**, SocialIQA del_tom -2.86pp
- 📉 距 DS 最远: SocialIQA direct -2.40pp, EmoBench (3 protocols) -8 ~ -11pp
...
| Hi-ToM | **+4.17** 🔥 | -1.16 ⚠️ | -0.50 | **+2.51** |
| SocialIQA | -0.05 | **+0.71** | +0.41 | +1.07 |
| ToMBench | +0.12 | **+0.86** | +0.51 | **+1.49** |
| **Sum** | **+3.99** | **+1.16** | **+0.84** | **+5.99** |

净: **+5.99pp 总和, +0.50pp 平均**, **9 升 3 降**(vs v3.4)

**v3.1 → v3.5 进化路径** (Hi-ToM cot, 距 DS 缩小 → v3.5 微回升):
- v3.1: -8.08pp
- v3.2 (1200 Hi-ToM 真实数据 + reward worker A-Z fix): -3.25pp
- v3.3 (Hi-ToM direct-style + EmoBench/SocialIQA 合成扩充): -1.25pp
- v3.4 (GPT-5.5 paraphrase + verified distillation): **-0.42pp** ⭐ peak
- **v3.5 (improved distill v2: pre-verify + voting + ontology): -1.58pp** (cot 微退，但 direct/del_tom 反超 DS)

**v3.5 单独反超 DS 的格**:
- Hi-ToM direct: 0.5900 vs DS 0.5825 (+0.75pp) ✅
- Hi-ToM del_tom: 0.7567 vs DS cot 0.7475 (+0.92pp，DS del_tom hang 不可比) ✅
